<x-app-layout>
    <div class="max-w-3xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8 py-6"
         x-data="{
            step: 1,
            typeContrat: '{{ old('type_contrat', $contract->type_contrat) }}',
            date_debut: '{{ old('date_debut', $contract->date_debut->format('Y-m-d')) }}',
            date_fin: '{{ old('date_fin', $contract->date_fin ? $contract->date_fin->format('Y-m-d') : '') }}',
            salaire: '{{ old('salaire_mensuel_brut', $contract->salaire_mensuel_brut) }}',
        ...
        <div>
            <h1 class="text-xl font-medium text-[#2C2C2A]">Modifier le contrat</h1>
            <p class="text-[15px] text-[#888780] mt-1">Contrat {{ $contract->numero_contrat }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl p-5 sm:p-6" style="border: 0.5px solid rgba(0,0,0,0.1);">
            ...
            <form method="POST" action="{{ route('employer.contracts.update', $contract) }}" class="space-y-6">
                @csrf
                @method('PUT')

                <section x-show="step === 1" x-transition.opacity.duration.200ms class="space-y-5">
                    <div>
                        <h2 class="text-base font-medium text-[#2C2C2A]">Type de contrat</h2>
                        <p class="text-[15px] text-[#888780] mt-1">L'employé reste inchangé, vous pouvez ajuster le type.</p>
                    </div>

                    <div>
                        <x-input-label :value="__('Employé')" />
                        <div class="mt-1 p-3 rounded-xl bg-[#F1EFE8] text-[15px] text-[#2C2C2A]" style="border: 0.5px solid rgba(0,0,0,0.08);">
                    ...
                    <div class="pt-2 flex justify-end">
                        <button type="button" @click="step = 2" class="btn btn-primary btn-md">Continuer</button>
                    </div>
                </section>
                ...
                <section x-show="step === 2" x-transition.opacity.duration.200ms class="space-y-5" x-cloak>
                    <div>
                        ...
                        <p class="text-[15px] text-[#888780] mt-1">Mettez à jour les informations contractuelles.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="periode_essai" class="block mt-1 w-full" type="text" name="periode_essai" :value="old('periode_essai', $contract->periode_essai)" />
                        </div>
                    </div>

                    @php $selectedAvantages = old('avantages', $contract->avantages ?? []); @endphp
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2">
                            @foreach(['Transport', 'Logement', 'Santé', 'Restauration'] as $avantage)
                                <label class="p-3 rounded-xl border border-[#D3D1C7] text-[15px] text-[#2C2C2A] flex items-center gap-2">
                                    <input type="checkbox" name="avantages[]" value="{{ $avantage }}" {{ in_array($avantage, $selectedAvantages) ? 'checked' : '' }}>
                                    <span>{{ $avantage }}</span>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <textarea id="clauses_specifiques" name="clauses_specifiques" rows="4" class="block mt-1 w-full">{{ old('clauses_specifiques', $contract->clauses_specifiques) }}</textarea>
                    </div>
